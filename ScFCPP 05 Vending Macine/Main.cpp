...
int main() {
	...
	slot1->addSnack(bounty); //ƒобавл€ем батончик в слот
	slot1->addSnack(bounty);

	SnackSlot* slot2 = new SnackSlot(10/*количество батончиков, которые помещаютс€ в слот*/);
	slot2->addSnack(snickers); //ƒобавл€ем батончик в слот
	slot2->addSnack(snickers);
	slot2->addSnack(snickers); 
	slot2->addSnack(snickers);

	machine1->addSlot(slot1); // ѕомещаем слот обратно в аппарат
	machine1->addSlot(slot2); // ѕомещаем слот обратно в аппарат
	...
	cout << "VM" << machine->getNumberVM() << "  Empty slots: " << machine->getEmptySlotsCount() << endl; // ƒолжно выводить количество пустых слотов дл€ снеков
	cout << "VM" << machine1->getNumberVM() << "  Empty slots: " << machine1->getEmptySlotsCount() << endl; // ƒолжно выводить количество пустых слотов дл€ снеков


	cout << "VM" << machine->getNumberVM() << "  Left snacks: " << machine->getLeftQtty() << endl; // ƒолжно выводить количество пустых слотов дл€ снек
	cout << "VM" << machine1->getNumberVM() << "  Left snacks: " << machine1->getLeftQtty() << endl; // ƒолжно выводить количество пустых слотов дл€ снек
